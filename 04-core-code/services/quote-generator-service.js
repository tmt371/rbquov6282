...
to publish an error event here.
        }
    }
...
= finalHtml.replace(
    ...
    _prepareTemplateData(quoteData, ui, 
f3Data) {
        ...
        const remote16chQty = (ui.f1.remote_1ch_qty === null) 
? totalRemoteQty : (totalRemoteQty - remote1chQty);
...
remote16chQty: remote16chQty || '',
    ...
    _formatCustomerInfo(f3Data) {
        let html 
= `<strong>${f3Data.customerName || ''}</strong><br>`;
...
const deliveryPriceClass = deliveryExcluded ? 'class="align-right is-excluded"' : 'class="align-right"';
  ...
  <td data-label="NO">${itemNumber++}</td>
        ...
        const removalDiscountedPrice = removalExcluded ? 0 
